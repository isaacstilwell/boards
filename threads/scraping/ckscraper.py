...
class CKScraper(BaseScraper):
    ...
    async def _get_threads(self, item_type: ItemType) -> List[ScrapedThread]:
        ...
        for partial in links:
            try:
                ...
                # CK gives an abnormal amount of bad responses, but i dont think it has to do
                # with blocking connections... just skip the thread if it goes wrong, and it'll
                # probably get scraped in the next rotation
    ...
    async def _extract_thread(self, url: str, item_type: ItemType) -> ScrapedThread:
        async with httpx.AsyncClient() as client:
            ...
            # CK sometimes includes authors in text like "Designed by <Name>". they are always in rich-text blocks
            designer = None
            ...
            for section in rt_sections:
                if (text_element := section.find(string=designed_by_text)):
            ...
            image_links = [f"https://{anchor["href"].replace("//", "")}" for anchor in thumbnails.find_all("a", class_="thumbnail")] if thumbnails else []

            # vendors are in one of the accordions, but the specific accordion has no more detail,
            # so just read all accordions (there are only 2-3) and check links against vendor_map
            # also start with cannonkeys becuase we're scraping the site
            vendors = ["CannonKeys"]
            ...
            for accordion in accordions:
                ...
                vendors.extend([self.vendor_map[link] for link in accordion_links if link in self.vendor_map])

            # CK is american
            currency = "USD"

            price_container = soup.find("div", class_="price-container")
            if price_container and (price_element := price_container.find("span", class_="price__current")):
            ...
            if product_label and (thread_type_text := product_label.find("span").text.lower()):
                # check for out of stock first, because all other forms of stock are instock
                if "out of stock" in thread_type_text or "completed" in thread_type_text:
            ...
            start_date = None
            end_date = None
            if (info_block := soup.find("div", class_="product-info-block")):
                date_label = info_block.find("p").text
                # CK dates are inconsistent and sometimes we'll have a 2 date string, other times 1 date
                # a single date is usually an end date unless the gb hasn't started yet, in which case it's
                # the start date
